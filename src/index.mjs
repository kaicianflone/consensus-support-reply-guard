...
import { rejectUnknown, getLatest, getPersonaSet, getDecisionByKey, writeArtifact, detectHardBlockFlags, aggregateVotes, updateReputations, makeIdempotencyKey } from 'consensus-guard-core/src/index.mjs';

const TOP = new Set(['board_id','reply_draft','constraints','persona_set_id']);
...
export async function handler(input, opts={}) {
  ...
  try {
    ...
    let ps=input.persona_set_id?await getPersonaSet(board_id,input.persona_set_id,statePath):await getLatest(board_id,'persona_set',statePath);
    if(!ps){ const g=await personaGen({board_id,task_context:{goal:'support guard',audience:'customers',risk_tolerance:'low',constraints:[],domain:'support'},n_personas:5,persona_pack:'comms'},{statePath}); if(g.error) return err(board_id,'PERSONA_GENERATION_FAILED',g.error.message); ps={persona_set_id:g.persona_set_id,personas:g.personas}; }
    const idem=makeIdempotencyKey({board_id,reply_draft:input.reply_draft,constraints:input.constraints||{},persona_set_id:ps.persona_set_id});
    const prior=await getDecisionByKey(board_id,idem,statePath); if(prior?.response) return prior.response;
    const votes=makeVotes(ps,input.reply_draft,input.constraints||{}); const ag=aggregateVotes(votes,{method:'WEIGHTED_APPROVAL_VOTE',approve_threshold:0.7});
    ...
    const p=await writeArtifact(board_id,'persona_set',{...ps,persona_set_id:crypto.randomUUID(),updated_at:timestamp,lineage:{parent_persona_set_id:ps.persona_set_id},personas},statePath);
    response.board_writes=[{type:'decision',success:true,ref:d.ref},{type:'persona_set',success:true,ref:p.ref}];
    return response;
